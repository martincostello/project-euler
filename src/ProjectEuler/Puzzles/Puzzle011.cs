...
namespace MartinCostello.ProjectEuler.Puzzles
{
    ...
    internal sealed class Puzzle011 : Puzzle
    {
        ...
        /// </summary>
        private static readonly int[][] Grid = BuildGrid();
        ...
        protected override int SolveCore(string[] args)
        {
            ...
            for (int x = 0; x < width; x++)
            {
                for (int y = 0; y < height; y++)
                {
                    if (y + 4 < height)
                    {
                        sequences.Add(new[] { Grid[x][y], Grid[x][y + 1], Grid[x][y + 2], Grid[x][y + 3] });
                    }

                    if (x + 4 < width)
                    {
                        sequences.Add(new[] { Grid[x][y], Grid[x + 1][y], Grid[x + 2][y], Grid[x + 3][y] });

                        if (y + 4 < height)
                        {
                            sequences.Add(new[] { Grid[x][y], Grid[x + 1][y + 1], Grid[x + 2][y + 2], Grid[x + 3][y + 3] });
                        }
        ...
        /// <summary>
        /// Builds the two-dimensional grid of values.
        /// </summary>
        /// <returns>The two-dimensional array of values.</returns>
